...
class Profile(models.Model):
    ...
    def notify_following(self, page_user):
        if self.user != page_user:
            Notification(notification_type=Notification.FOLLOWED_USER,
                from_user=self.user,
                to_user=page_user).save()

    def unotify_following(self, page_user):
        if self.user != page_user:
            Notification.objects.filter(notification_type=Notification.FOLLOWED_USER,
                from_user=self.user,
                to_user=page_user).delete()
